...
rott5$train_01 = 1  # Train data indicator
gbsg5$train_01 = 0 
rott5$size = gbsg5$size = NULL

#' Append rott5 and gbsg5

dfx = bind_rows(rott5, gbsg5)

dfx5  = dfx %>% select(c(pid, train_01, year, ryear,rfs, csize, nodes2, nodes3, grade3, pgr3))
head(dfx5)
tail(dfx5)

efit1 = coxph(Surv(ryear, rfs) ~ csize + nodes2 + nodes3 + grade3, data = dfx5, subset= dfx5$train_01 == 1)
efit1
rm(efit1)
...
task1 = TaskSurv$new(id = "task1", backend = dfx5, time = "ryear", event = "rfs", type="right")
selected_features = c("csize", "nodes2", "nodes3", "grade3")

# Filter the task to use only the selected features
task1$select(selected_features)
task1$col_roles$group = "pid"
# task1$col_roles$weight = "wt"
# task1$col_roles$stratum = "grade3" # Defines stratification variables for resampling strategies.
# task1$col_roles$order = "time_order"
...
# Perform resampling (e.g., 3-fold cross-validation) and compute prediction 
resampling = rsmp("cv", folds = 3)
rr = resample(task1, learner, resampling)

# Collect predictions
prediction = rr$prediction()


### prediction = learner$predict(task1, row_ids = split$test)  # prediction on test data
...
# D-Calibration to evaluate a model’s calibration 
   ...
   perf_score = prediction$score(msrs(c("surv.graf", "surv.rcll", "surv.cindex", "surv.dcalib")))
...
task1_filtered = task1$clone(deep = TRUE)$filter(which(condition))
...
### Validation data
# Harrell's C
harrell_C_gbsg5 <- concordance(Surv(ryear, rfs) ~ td_lp, 
                               gbsg5, 
                               reverse = TRUE)
# Uno's C
Uno_C_gbsg5 <- concordance(Surv(ryear, rfs) ~ td_lp, 
                           gbsg5, 
                           reverse = TRUE,
                           timewt = "n/G2")
alpha <- .05
